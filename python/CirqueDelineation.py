...
Min_Cir_Index = arcpy.GetParameter(6)
Min_steep_slope_percent = arcpy.GetParameter(7)
Min_gentle_slope_percent = arcpy.GetParameter(8)
Max_backwall_slope =  arcpy.GetParameter(9)
Min_backwall_slope =  arcpy.GetParameter(10)

Out3DProfiles = arcpy.GetParameterAsText(11)
OutKnickpoints = arcpy.GetParameterAsText(12) ##the turning points for cirques
OutCirques = arcpy.GetParameterAsText(13)
...
#arcpy.CopyFeatures_management(TmpStream, "c:\\temp\\outStreamAll.shp")
...
for id in range(linecount):
    ...
    arcpy.AddMessage("Processing first order stream: " + str(id)+ " out of " + str(linecount))
            ...
            if meetcount > 0:                
                ...
                if (steep_percent > float(Min_steep_slope_percent) and gentle_percent > float(Min_gentle_slope_percent)): ##Only count the steep percent of >30% within the watershed, need to make a parameter for this value
                    ...
                    if (slopepolycount>0 and status): ##meet the cirque standards
                        arcpy.cartography.SmoothPolygon(SlopeBnd, smoothBnd, "PAEK", smoothtolerance)
                        ...
                        meetcount2  = 0
                        with arcpy.da.UpdateCursor(smoothBnd, ['SHAPE@LENGTH', 'SHAPE@AREA', 'LineID', 'PointID',"CirIndex"]) as cursor:
                            for row in cursor:
                                length = float(row[0])
                                area = float(row[1])
                                Circularindex = (4.0 * 3.14 * area) / (length * length)
